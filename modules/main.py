...
def load_dataset(syn_aug= True): 
    ...
    """
        Load dataset.
    """
    pretrain  = pickle.load(open("../data/stsallrmf.p","rb"), encoding='utf-8')#[:-8]
    train     = pickle.load(open("../data/semtrain.p",'rb'),encoding="utf-8")
    test      = pickle.load(open("../data/semtest.p",'rb'),encoding="utf-8")
...
def test_network(sess, network, test):
    ...
    feed_dict={ network.x1:          test[0],
                ...
                network.len2:        test[3],
                network.y:           test[4],
                network.batch_size:  np.array(test[0]).shape[0]} #np.array(test[0]).shape[0]
    print("batch_size",np.array(test[0]).shape[0])
...
def main():
    ...
    test     = data_util.prepare_data(test, maxlen=args.max_length, training=True)
    # Create network
    network = cnn_lstm.SiameseLSTMCNN(
